...
// in simulator we redefine to format for SAUS Developer console
...
{\
	NSString * exceptionName = [@"org.saus." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.saus";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UISAUSNativeItemNone = -1, 
	UISAUSNativeItemSpinner = -2,
	UISAUSNativeItemProgressBar = -3,
	
	UISAUSNativeItemSlider = -4,
	UISAUSNativeItemSwitch = -5,
	UISAUSNativeItemMultiButton = -6,
	UISAUSNativeItemSegmented = -7,
	
	UISAUSNativeItemTextView = -8,
	UISAUSNativeItemTextField = -9,
	UISAUSNativeItemSearchBar = -10,
	
	UISAUSNativeItemPicker = -11,
	UISAUSNativeItemDatePicker = -12,
	
	UISAUSNativeItemInfoLight = -13,
	UISAUSNativeItemInfoDark = -14,
	
	UISAUSNativeItemDisclosure = -15,
	
	UISAUSNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all SAUS instances of
